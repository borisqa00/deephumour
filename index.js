...
const deephumournetDemo = async () => {
  ...
  const ricardoElement = document.getElementById('ricardo');
  if (ricardoElement.complete && ricardoElement.naturalHeight !== 0) {
    predict(ricardoElement);
    ricardoElement.style.display = '';
  } else {
    ricardoElement.onload = () => {
      predict(ricardoElement);
      ricardoElement.style.display = '';
    }
  }
